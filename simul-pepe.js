...
The simulation below deploys and tests the pepe-single-faktory-stxer contract which
references the real deployed contracts:
- pepe Token: SP2VG7S0R4Z8PYNYCAQ04HCBX1MH75VT11VXCWQ6G.built-on-bitcoin-stxcity
- pepe Pool: SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.pepe-faktory-pool
*/
...
const pepe_PROVIDER = "SP3YM5YRTKHTWRC82K5DZJBY9XW0K4AX0P9PM5VSH";
const SBTC_USER_1 = "SP24MM95FEZJY3XWSBGZ5CT8DV04J6NVM5QA4WDXZ";
const SBTC_USER_2 = "SPG90HHG9DQSZRC921S56CRYJ89K229YDE0N5SN1";
...
SimulationBuilder.new()
  ...
  // Deploy the pepe-single-faktory-stxer contract (which references the real contracts)
  .addContractDeploy({
    contract_name: "pepe-single-faktory-stxer",
    source_code: fs.readFileSync(
      "./contracts/pepe-single-faktory-stxer.clar",
      "utf8"
    ),
    clarity_version: ClarityVersion.Clarity3,
  })

  // Initialize the pepe pool using our deployed contract
  .withSender(pepe_PROVIDER)
  .addContractCall({
    contract_id: `${DEPLOYER}.pepe-single-faktory-stxer`,
    function_name: "initialize-pool",
    function_args: [uintCV(1000000000000)], // Your exact amount (pepe tokens)
  })

  // Check pool info after initialization
  .addEvalCode(`${DEPLOYER}.pepe-single-faktory-stxer`, "(get-pool-info)")
  ...
  .addEvalCode(
    `${DEPLOYER}.pepe-single-faktory-stxer`,
  ...
  // TEST: Random user tries to withdraw remaining pepe tokens (should fail - not authorized)
  .withSender(RANDOM_USER)
  .addContractCall({
    contract_id: `${DEPLOYER}.pepe-single-faktory-stxer`,
  ...
  // TEST: pepe provider withdraws remaining pepe tokens (should work)
  .withSender(pepe_PROVIDER)
  .addContractCall({
    contract_id: `${DEPLOYER}.pepe-single-faktory-stxer`,
    function_name: "withdraw-remaining-token",
  ...
  // TEST: Try pepe provider depositing sBTC (should fail - unauthorized)
  .withSender(pepe_PROVIDER)
  .addContractCall({
    contract_id: `${DEPLOYER}.pepe-single-faktory-stxer`,
    function_name: "deposit-sbtc-for-lp",
