...
const contractName = "pepe-single-faktory";

describe("Pepe Single Faktory Contract Tests", () => {
  beforeEach(() => {
    simnet.setEpoch("3.0");
  ...
  describe("Contract Configuration & Constants", () => {
    it("should return correct contract configuration", () => {
      ...
      expect(result.result).toStrictEqual(
        Cl.tuple({
          ft: Cl.contractPrincipal(
            simnet.deployer,
            "tokensoft-token-v4k68639zxz"
          ),
          pool: Cl.contractPrincipal(simnet.deployer, "pepe-faktory-pool"),
