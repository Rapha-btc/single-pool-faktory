...
(define-public (initialize-pool (token-amount uint))
  (begin
    ...
    (try! (contract-call? .tokensoft-token-v4k68639zxz 
    ...
    (print {
      ...
      ft: .tokensoft-token-v4k68639zxz,
...
(define-public (deposit-sbtc-for-lp (lp-amount uint))
    (let (
          ...
          (lp-result (try! (as-contract (contract-call? .pepe-faktory-pool 
...
(define-public (withdraw-lp-tokens)
  (let ((unlock-block (+ (var-get creation-block) LOCK_PERIOD))
    ...
    (let ((remove-result (try! (as-contract (contract-call? .pepe-faktory-pool 
        ...
        (try! (as-contract (contract-call? .tokensoft-token-v4k68639zxz 
...
(define-read-only (get-quote-for-lp (lp-amount uint))
  (contract-call? .pepe-faktory-pool 
