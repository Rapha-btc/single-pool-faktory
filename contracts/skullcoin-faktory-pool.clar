...
;; Define LP token
(define-fungible-token sBTC-SKULL)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? sBTC-SKULL amount sender recipient))
    (match memo
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance sBTC-SKULL who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply sBTC-SKULL))
)
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (ft-mint? sBTC-SKULL dk-d sender))
    (print {
      type: "add-liquidity",
      sender: sender,
      token-a: .sbtc-token,
      token-a-amount: dx-d,
      token-b: .skullcoin-stxcity,
      token-b-amount: dy-d,
      lp-tokens: dk-d,
      pool-reserves: (get-reserves-quote),
      pool-contract: CONTRACT,
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? sBTC-SKULL dk-d sender))
    (try! (as-contract (contract-call? .sbtc-token
...
(define-read-only (get-reserves-quote)
  (let (
      (reserves (get-reserves))
      (supply (ft-get-supply sBTC-SKULL))
    )
    {
      dx: (get a reserves),
      dy: (get b reserves),
...
;;   (try! (add-liquidity u735000))
;;   ;; Transfer additional token B to achieve desired ratio
;;   (try! (contract-call?
;;     .skullcoin-stxcity
;;     transfer u792755065936611 tx-sender CONTRACT none
;;   ))
;;   (print {
