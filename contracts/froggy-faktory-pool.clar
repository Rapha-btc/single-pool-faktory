...
;; Title: sBTC-FROGGY lp-token (Single Pool)
      ;; Version: 1.0.0
      ...
      ;; Define LP token
      (define-fungible-token sBTC-FROGGY)
      ...
      (define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
          (begin
              ...
              (try! (ft-transfer? sBTC-FROGGY amount sender recipient))
      ...
      (define-read-only (get-balance (who principal))
          (ok (ft-get-balance sBTC-FROGGY who)))
  
      (define-read-only (get-total-supply)
          (ok (ft-get-supply sBTC-FROGGY)))
      ...
      (define-public (swap-a-to-b (amount uint) (min-y-out uint))
          (let (
              ...
              (try! (contract-call? .sbtc-token transfer amount sender CONTRACT none))
              ;; Transfer token B to sender
              (try! (as-contract (contract-call? .frog-faktory transfer dy-d CONTRACT sender none)))
              (print {
                  type: "buy",
                  sender: sender,
                  token-in: .sbtc-token,
                  amount-in: amount,
                  token-out: .frog-faktory,
                  amount-out: dy-d,
      ...
      (define-public (swap-b-to-a (amount uint) (min-y-out uint))
          (let (
              ...
              (try! (contract-call? .frog-faktory transfer amount sender CONTRACT none))
              ;; Transfer token A to sender
              (try! (as-contract (contract-call? .sbtc-token transfer dy-d CONTRACT sender none)))
              (print {
                  type: "sell",
                  sender: sender,
                  token-in: .frog-faktory,
                  amount-in: amount,
      ...
      (define-public (add-liquidity (amount uint))
          (let (
              ...
              (dy-d (get dy delta))
              (dk-d (get dk delta)))
              (try! (contract-call? .sbtc-token transfer dx-d sender CONTRACT none))
              (try! (contract-call? .frog-faktory transfer dy-d sender CONTRACT none))
              (try! (ft-mint? sBTC-FROGGY dk-d sender))
              (print {
                  type: "add-liquidity",
                  sender: sender,
                  token-a: .sbtc-token,
                  token-a-amount: dx-d,
                  token-b: .frog-faktory,
                  token-b-amount: dy-d,
                  lp-tokens: dk-d,
                  pool-reserves: (get-reserves-quote),
                  pool-contract: CONTRACT
      ...
      (define-public (remove-liquidity (amount uint))
          (let (
              ...
              (dy-d (get dy delta))
              (dk-d (get dk delta)))
              (try! (ft-burn? sBTC-FROGGY dk-d sender))
              (try! (as-contract (contract-call? .sbtc-token transfer dx-d CONTRACT sender none)))
              (try! (as-contract (contract-call? .frog-faktory transfer dy-d CONTRACT sender none)))
              (print {
      ...
      (define-private (get-reserves)
          { 
            a: (unwrap-panic (contract-call? .sbtc-token get-balance CONTRACT)), 
            b: (unwrap-panic (contract-call? .frog-faktory get-balance CONTRACT))
          })
      ...
      (define-read-only (get-reserves-quote)
          (let (
              (reserves (get-reserves))
              (supply (ft-get-supply sBTC-FROGGY)))
              {
                dx: (get a reserves),
                dy: (get b reserves),
                dk: supply
    ...
    ;;     (try! (add-liquidity u100000))
    ;;     ;; Transfer additional token B to achieve desired ratio
    ;;     (try! (contract-call?
    ;;       .frog-faktory
    ;;       transfer u11428708900000 tx-sender CONTRACT none
    ;;     ))
    ;;     (print {
