...
(define-public (initialize-pool (token-amount uint))
  (begin
    ...
    (try! (contract-call? .skullcoin-stxcity 
...
(define-public (deposit-sbtc-for-lp (lp-amount uint))
    (let (
          ...
          (lp-result (try! (as-contract (contract-call? .skullcoin-faktory-pool 
...
(define-public (withdraw-lp-tokens)
  (let ((unlock-block (+ (var-get creation-block) LOCK_PERIOD))
    ...
    (let ((remove-result (try! (as-contract (contract-call? .skullcoin-faktory-pool 
        ...
        (try! (as-contract (contract-call? .skullcoin-stxcity 
...
(define-read-only (get-quote-for-lp (lp-amount uint))
  (contract-call? .skullcoin-faktory-pool 
