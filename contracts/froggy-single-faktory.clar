...
(define-public (initialize-pool (token-amount uint))
  (begin
    ...
    (try! (contract-call? .frog-faktory 
...
(define-public (deposit-sbtc-for-lp (lp-amount uint))
    (let (
          ...
          (lp-result (try! (as-contract (contract-call? .froggy-faktory-pool 
...
(define-public (withdraw-lp-tokens)
  (let ((unlock-block (+ (var-get creation-block) LOCK_PERIOD))
    ...
    (let ((remove-result (try! (as-contract (contract-call? .froggy-faktory-pool 
        ...
        (try! (as-contract (contract-call? .frog-faktory 
...
(define-read-only (get-quote-for-lp (lp-amount uint))
  (contract-call? .froggy-faktory-pool 
